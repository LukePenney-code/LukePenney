...
BWAPI::Unitset Tools::GetAllUnitsOfType(BWAPI::UnitType type)
{
    BWAPI::Unitset set;
    // For each unit that we own
    for (auto& unit : BWAPI::Broodwar->self()->getUnits())
...
BWAPI::Unit Tools::GetIdleWorker()
{
    ...
    for (auto& unit : BWAPI::Broodwar->self()->getUnits())
    {
        // if the unit is of the correct type, and it actually has been constructed, return it
        if (unit->getType() == type && unit->isCompleted()
            && (unit->isIdle() || unit->isGatheringGas() || unit->isGatheringMinerals() || unit->isCarryingGas() || unit->isCarryingMinerals()))
        {
            return unit;
        }
    }
...
bool Tools::BuildBuilding(BWAPI::UnitType type, Base& base)
{
    BWAPI::UnitType builderType = type.whatBuilds().first;
    BWAPI::Unit builder = base.getWorker();
    if (!builder || builderType != builder->getType()) { return false; }

    // Get a location that we want to build the building next to
    BWAPI::TilePosition desiredPos = base.getPosition();
...
bool Tools::BuildBuilding(BWAPI::UnitType type)
{
    ...
    BWAPI::Unit builder = Tools::GetIdleWorker();
...
bool Tools::BuildBuilding(BWAPI::UnitType type, BWAPI::TilePosition loc)
{
    // Get the type of unit that is required to build the desired building
    BWAPI::UnitType builderType = type.whatBuilds().first;
    ...
    // Get a location that we want to build the building next to
    BWAPI::TilePosition desiredPos = loc;
